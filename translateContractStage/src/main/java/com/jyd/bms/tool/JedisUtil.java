...
public class JedisUtil {
	...
	static {
		...
		config.setMaxTotal(Environment.getRedisMaxTotal());
		// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
		config.setMaxIdle(Environment.getRedisMaxIdle());
		// 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
		config.setMaxWaitMillis(Environment.getRedisMaxWaitMillis());
		// 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
		config.setTestOnBorrow(true);

		// redis如果设置了密码：
		if (Environment.getRedisPassword().equals("") == false) {
			jedisPool = new JedisPool(config, Environment.getRedisServer(), Environment.getRedisPort(), 10000,
					Environment.getRedisPassword());
		} else {
			// redis未设置了密码：
			jedisPool = new JedisPool(config, Environment.getRedisServer(), Environment.getRedisPort());
		}
	}
	...
	public static void main(String[] args) {
		...
		strings.set("OK", "nnnn");
		System.out.println("-----" + strings.get("74e8a6925f714f36b2aeda61b19f1200"));
