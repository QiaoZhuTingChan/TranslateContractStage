...
public abstract class HibernateBaseTemplate<T> {
	...
	protected final List getNamedQueryResult(String queryName) throws DAOException {
		Query query = getNamedQuery(queryName);
		try {
			return getQueryResults(query);
		} catch (HibernateException ex) {
			throw new DAOException(
					"Query objects (queryName:" + queryName + ", hql:" + query.getQueryString() + ") occur error.", ex);
		}
	}
