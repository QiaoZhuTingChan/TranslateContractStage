...
public class UserDAOImpl extends HibernateBaseTemplate<User> implements UserDAO {
	...
	public List<OnlineUserDTO> getAllOnlineUser(List<Integer> listEmployeeId) throws DAOException {
		...
		hql = "select new com.jyd.bms.dto.OnlineUserDTO(u.employee.id,u.loginName, u.employee.name, u.employee.department.departmentName, u.employee.department.store.shortName) from User u where u.employee.id in (:listEmployeeId)";
		map.put("listEmployeeId", listEmployeeId);
	...
	public List<User> getUsersByEmployeeList(List<Employee> employeeList) throws DAOException {
		...
		hql.append(" from User where employee in (:employeeList)");
		map.put("employeeList", employeeList);
		
		List<User> list = super.getQueryResult(hql.toString(), map);
