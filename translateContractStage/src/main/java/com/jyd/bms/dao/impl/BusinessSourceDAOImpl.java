package com.jyd.bms.dao.impl;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import org.springframework.stereotype.Repository;

import com.jyd.bms.bean.BusinessSource;
import com.jyd.bms.dao.BusinessSourceDAO;
import com.jyd.bms.tool.exception.DAOException;

@Repository
public class BusinessSourceDAOImpl extends HibernateBaseTemplate<BusinessSource> implements BusinessSourceDAO {

	public int getBusinessSourceCount(String condition) throws DAOException {
		String hql = "";
		if (condition.equals("")) {
			hql = "select count(*) from BusinessSource";
			List<Long> lstCount = super.getQueryResult(hql);
			return lstCount.get(0).intValue();
		} else {
			hql = "select count(*) from BusinessSource where businessSource like :condition";
			@SuppressWarnings("rawtypes")
			Map map = new HashMap();
			map.put("condition", "%" + condition + "%");
			List<Long> lstCount = super.getQueryResult(hql, map);
			return lstCount.get(0).intValue();
		}
	}

	public List<BusinessSource> getPagingBusinessSource(int firstResult, int maxResults, String condition) throws DAOException {
		String hql = "";
		@SuppressWarnings("rawtypes")
		Map map = new HashMap();
		if (condition.equals("")) {
			hql = "from BusinessSource";
		} else {
			hql = "from BusinessSource where businessSource like :condition";
			map.put("condition", "%" + condition + "%");
		}
		return super.getPagingQueryResult(hql.toString(), map, firstResult, maxResults);
	}

	public List<BusinessSource> getAllBusinessSource() throws DAOException {
		String hql = "";
		hql = "from BusinessSource";
		return super.getQueryResult(hql.toString());

	}

}
